...
(function(){
  ...
  // --- Character portrait fix (fallback) ---
  function fixPortraits(){
    ...
    if(!img) return;
    img.src = toRaw(img.getAttribute('data-src') || img.getAttribute('src') || '');
    img.onerror = ()=>{
      img.onerror = null;
  ...
  function injectGlobalTrackers(){
    ...
    const host = document.querySelector('#appHeader, header, .top-bar, body');
  ...
  function enhanceThemeCards(){
    ...
    cards.forEach((card,i)=>{
      ...
      const checked = store.get(impKey,false) ? 'checked' : '';
      imp.innerHTML = `<input type=\"checkbox\" ${checked}> <span class=\"meter-label\">Improvement Achieved<\/span>`;
      ...
      // weakness burn (🔥 buttons already exist in tags if present)
  ...
  // --- Core Moves & Combos (always visible) ---
  const combos = {
    Slay: { tags: ['Watch Me Shine Anyway','Hot and Crunchy','Heels-first Entrance','Sugar Rush Combo'], power: 4, need: 2 },
    Care: { tags: ['I Shield Who I Love','My Drag Babies Come First','I\\'ve Survived Worse Than You'], power: 3, need: 2 },
    'Strike A Pose': { tags: ['You Don\\'t Get to Dull My Sparkle','Watch Me Shine Anyway','Legendary Name Recognition'], power: 4, need: 2 },
  };
