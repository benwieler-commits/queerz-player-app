...
// Store as Set of tag NAME strings (not full objects)
const mcCreatedStatusTags = new Set();
const mcCreatedStoryTags = new Set();

// Flag to prevent broadcast loops
let isProcessingMcTags = false;

// Listen for MC tag updates from firebase-config.js
// FIXED: Listen for BOTH event names for compatibility
['mc-tag-update', 'mc-tags-updated'].forEach(eventName => {
  document.addEventListener(eventName, e => {
    const detail = e.detail || {};
    // Handle both old format (statusTags/storyTags) and new format (currentStatuses/storyTags)
    const statusTags = detail.statusTags || detail.currentStatuses || [];
    const storyTags = detail.storyTags || [];
    
    console.log(`🏷️ MC tags received via ${eventName}:`, { statusTags, storyTags });
    
    // Set flag to prevent immediate re-broadcast
    isProcessingMcTags = true;
    
    // Track all MC-created tags by NAME (extract name if object)
    statusTags.forEach(tag => {
      if (tag) {
        const tagName = typeof tag === 'string' ? tag : (tag.name || tag.rawString || String(tag));
        mcCreatedStatusTags.add(tagName);
        console.log('   📌 Tracking MC status tag:', tagName);
      }
    });
    ...
    storyTags.forEach(tag => {
      if (tag) {
        const tagName = typeof tag === 'string' ? tag : (tag.name || tag.rawString || String(tag));
        mcCreatedStoryTags.add(tagName);
        console.log('   📖 Tracking MC story tag:', tagName);
      }
    });
    
    // Reset flag after short delay to allow normal broadcasts again
    setTimeout(() => {
      isProcessingMcTags = false;
    }, 500);
  });
});
...
export async function broadcastPlayerToMc(characterData) {
  ...
  // Prevent broadcast loop when processing MC tags
  if (isProcessingMcTags) {
    console.log('⏸️ Skipping broadcast - currently processing MC tags');
    return false;
  }
 ...
 * FIXED: Handle both string tags and object tags
 */
function filterPlayerCreatedTags(tags, mcSet) {
  ...
  return tags.filter(tag => {
    ...
    const tagName = typeof tag === 'string' ? tag : (tag.name || tag.rawString || String(tag));
    const isMcCreated = mcSet.has(tagName);
    if (isMcCreated) {
      console.log('   🚫 Filtering out MC-created tag:', tagName);
    }
    return !isMcCreated;
  });
}
...
console.log('   🔄 Handles both string and object tags');
